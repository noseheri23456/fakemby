...
# 5. 测试图片重定向模式（GET /emby/Items/{ItemId}/Images/{Type}�?echo "5. 测试图片重定向模�?.."
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$BASE_URL/emby/Items/$ITEM_ID/Images/Primary" \
  -H "X-Emby-Token: $TOKEN")

if [ "$HTTP_CODE" = "302" ] || [ "$HTTP_CODE" = "200" ]; then
  echo "�?图片重定向成�?(HTTP $HTTP_CODE)"
else
  echo "�?图片重定向失�?(HTTP $HTTP_CODE)"
fi
...
# 7. 测试带索引的图片请求（GET /emby/Items/{ItemId}/Images/{Type}/{Index}�?echo "7. 测试带索引的图片请求..."
