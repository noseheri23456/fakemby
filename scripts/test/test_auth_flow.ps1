...
# 步骤 1: 获取公开系统信息（无需认证�?Write-Host "步骤 1️⃣: 获取公开系统信息（无需认证�? -ForegroundColor Yellow
try {
...
# 步骤 2: 获取可用用户列表（无需认证�?Write-Host "步骤 2️⃣: 获取可用用户列表（无需认证�? -ForegroundColor Yellow
try {
    $response = Invoke-WebRequest -Uri "$serverUrl/emby/Users/Public" -ErrorAction Stop
    Write-Host "�?成功 (200)" -ForegroundColor Green
    $data = $response.Content | ConvertFrom-Json
    Write-Host "  可用用户�? $($data.Users.Count)"
...
try {
    ...
    Write-Host "  用户�? $($data.User.Name)"
    Write-Host "  是否管理�? $($data.User.IsAdmin)"
} catch {
...
# 步骤 4: �?Token 访问受保护端点（多种方式测试�?Write-Host "步骤 4️⃣: �?Token 访问受保护端�? -ForegroundColor Yellow
...
# 步骤 5: 获取用户媒体�?Write-Host "步骤 5️⃣: 获取用户媒体�? -ForegroundColor Yellow
try {
...
try {
    ...
    if ($data.Items.Count -gt 0) {
        ...
        if ($data.Items.Count -gt 3) {
            Write-Host "    ... 以及其他 $($data.Items.Count - 3) �?
        }
    }
} catch {
    Write-Host "�?失败: $($_.Exception.Response.StatusCode)" -ForegroundColor Red
...
Write-Host "2. 重新添加服务�?http://localhost:8096"
