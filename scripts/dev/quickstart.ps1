# FakEmby 快速启动脚本（一键启�?+ 导入测试数据�?
Write-Host "╔════════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "�?             FakEmby 服务�?- 快速启�?                        �? -ForegroundColor Cyan
Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "1️⃣  清理旧进�?.." -ForegroundColor Yellow
Get-Process fakemby -ErrorAction SilentlyContinue | Stop-Process -Force
Start-Sleep -Seconds 1

Write-Host "2️⃣  编译服务�?.." -ForegroundColor Yellow
$env:CGO_ENABLED = 0
...
Write-Host "3️⃣  启动服务�?.." -ForegroundColor Yellow
...
Write-Host "  �?导入电影库：$($result.imported) �? -ForegroundColor Green
...
$response = Invoke-WebRequest -Uri "http://localhost:8096/api/admin/import" `
    ...
    -Body $json -ErrorAction Stop
$result = $response.Content | ConvertFrom-Json
Write-Host "  �?导入电视剧库�?($result.imported) �? -ForegroundColor Green
...
Write-Host "════════════════════════════════════════════════════════════════" -ForegroundColor Green
Write-Host ""
Write-Host "📱 �?RodelPlayer 中连接：" -ForegroundColor Cyan
...
Write-Host "   �?服务器窗口最小化运行在后�? -ForegroundColor Yellow
